...
{
    public static List<BSPNode> TraverseGraphToExtractLowestLeafes(BSPNode parentNode)
    {
        Queue<BSPNode> nodesToCheck = new Queue<BSPNode>();
        List<BSPNode> listToReturn = new List<BSPNode>();
        if (parentNode.ChildrenNodeList.Count == 0)
        {
            return new List<BSPNode>() { parentNode };
        }
